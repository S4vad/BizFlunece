...
export default function InfluencerList() {
  ...
  useEffect(() => {
    ...
    const fetchFavoriteInfluencers = async () => {
      try {
        ...
        console.log("Error fetching favorite influencers", error);
      }
    };
  ...
  useEffect(() => {
    const filtered = influencers.filter((influencer) => {
      ...
      const category = influencer.influencerCategory?.join(' ')?.toLowerCase() ?? '';
      const bio = influencer.bio?.toLowerCase() ?? '';
      const platforms = influencer.platform?.map(p => p.platform?.toLowerCase()).join(' ') ?? '';
  ...
  const uniqueCategories = Array.from(
    new Set(
      influencers.flatMap(inf => inf.influencerCategory || [])
    )
  ).filter(Boolean);
        ...
        <div className="mb-8 rounded-xl bg-white p-6 shadow-sm">
          ...
          {filteredInfluencers.length > 0 ? (
            ...
            <div className="rounded-lg bg-gray-100 p-8 text-center">
              <p className="text-gray-500">No influencers match your search criteria</p>
              <button 
                onClick={() => {
                  setSearchQuery("");
